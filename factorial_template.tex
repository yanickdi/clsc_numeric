...
\newcommand\tabrotate[1]{\begin{turn}{90}\rlap{ {% raw %}#{% endraw %}1}\end{turn}} 
...
%}
{% set table_left = {'delta': '$\\delta$', 'tau':'tau', 'cn': 'cn', 'a':'a'} %}
        ...
        \begin{center}
            \begin{small}
                \begin{tabular}{cc|ccccc}
                    ...
                    \hline
                    \multirow{ {{pars|length}} }{*}{\tabrotate{ {{table_left[table]}} }}
                    {% for par in pars %}
                        &{{par}}&{{at(table, par)}}\\
                    {% endfor %}
                \end{tabular}
